...
function Home(props) {
  ...
  const getData = async () => {
    try {
      const response = await fetch(
        `${API_URL}/api/restaurants/filters/dinamicos/${position[0]}/${position[1]}/${
          distanceRes * 1000 || 4000
        }/${limitRes || 5}`,
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({ queryString, categoriesList }),
        }
      );
  ...
  const getCategories = async () => {
    try {
      const response = await fetch(
        `${API_URL}/api/restaurants/unique/categories/${position[0]}/${position[1]}/${
          distanceRes * 1000 || 4000
        }/${limitRes || 5}`
      );
        ...
        <div className="barra-busqueda-home">
          ...
          <div className="barra-busqueda-numbers-container">
            <div className="barra-busqueda-parejita">
              <label className="busqueda-numbers-labels" htmlFor="limit">
                Nº
              </label>
              <input
                className="barra-busqueda-numbers-input"
            ...
            <div className="barra-busqueda-parejita">
              <label className="busqueda-numbers-labels" htmlFor="distance">
                Distancia
              </label>
              <input
                className="barra-busqueda-numbers-input"
        ...
        <div className="home-map-container">
          {loading ? (
            <div className="loader-container">
              <DotLoader color={"#4682b6"} loading={loading} size={50} />
            </div>
          ) : (
          ...
          {availableCategories.map((categoria, index) => {
            return (
              <div key={index} className="categoria-home">
                <input
                  className="categoria-checker-input"
              ...
              <>
                {restaurantList.length <= 0 && (
                  <p className="sin-resultados">
                    ...
                    <br /> ¿Por qué no pruebas a aumentar la distancia?
                  </p>
                )}
